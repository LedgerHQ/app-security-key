...
from ..utils import generate_random_bytes, generate_make_credentials_params, \
    ctap2_get_assertion, Nav, fido_known_app
...
def test_client_pin_check_not_set(client):
    for client_pin in [client.client_pin_v1, client.client_pin_v2]:
        ...
        pin_auth = b""
        args = generate_make_credentials_params(client, pin_uv_param=pin_auth, pin_uv_protocol=client_pin.protocol.VERSION)
        with pytest.raises(CtapError) as e:
            client.ctap2.make_credential(args, will_fail=True)
        assert e.value.code == CtapError.ERR.PIN_NOT_SET

        with pytest.raises(CtapError) as e:
            client.ctap2.make_credential(args, navigation=Nav.USER_REFUSE, will_fail=True)
        ...
        with pytest.raises(CtapError) as e:
            client.ctap2.get_assertion(t.args.rp["id"], args.client_data_hash,
                                       allow_list,
...
def test_get_uv_token(client):
    for client_pin in [client.client_pin_v1, client.client_pin_v2]:
        ...
        token = client_pin.get_uv_token(ClientPin.PERMISSION.MAKE_CREDENTIAL,
                                        args.rp["id"])
        pin_auth = client_pin.protocol.authenticate(token, args.client_data_hash)

        bad_args = deepcopy(args)
        bad_args.rp = {"id": args.rp["id"] + ".scam"}
        with pytest.raises(CtapError) as e:
            client.ctap2.make_credential(bad_args, will_fail=True)
        assert e.value.code == CtapError.ERR.PIN_AUTH_INVALID
...
def test_use_pin(client):
    for client_pin in [client.client_pin_v1, client.client_pin_v2]:
        ...
        args = generate_make_credentials_params(client, pin_uv_protocol=client_pin.protocol.VERSION)
        ...
        args.pin_uv_param = client_pin.protocol.authenticate(bad_token, args.client_data_hash)
        ...
        args.pin_uv_protocol = 3
        # Check should use correct protocol
        with pytest.raises(CtapError) as e:
            client.ctap2.make_credential(args, will_fial=True)
        assert e.value.code == CtapError.ERR.INVALID_PARAMETER
        ...
        args.pin_uv_protocol = client_pin.protocol.VERSION
...
def test_client_pin_unique_token(client):
    for client_pin in [client.client_pin_v1, client.client_pin_v2]:
        ...
        args = generate_make_credentials_params(client, pin_uv_param=pin_auth, pin_uv_protocol=client_pin.protocol.VERSION, client_data_hash=client_data_hash)
        ...
        args = generate_make_credentials_params(client, pin_uv_protocol=client_pin.protocol.VERSION)
        pin_auth = client_pin.protocol.authenticate(token_a, args.client_data_hash)
        args.pin_uv_param = pin_auth
        with pytest.raises(CtapError) as e:
            client.ctap2.make_credential(args, will_fail=True)
        assert e.value.code == CtapError.ERR.PIN_AUTH_INVALID
        ...
        args.pin_uv_param = client_pin.protocol.authenticate(token_b, client_data_hash)
        client.ctap2.make_credential(args)
