...
#define CTAP2_MIN_USER_ID_SIZE 1
...
static int parse_makeCred_authnr_user(cbipDecoder_t *decoder, cbipItem_t *mapItem) {
    ...
    // The range for the user ID length (userIdLen) is 1 to 64 bytes
    if ((ctap2RegisterData->userIdLen < CTAP2_MIN_USER_ID_SIZE) ||
        (ctap2RegisterData->userIdLen > CTAP2_MAX_USER_ID_SIZE)) {
